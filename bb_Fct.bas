...
Set Rs = QQRs("Select FldNm & ' ' & ? from [Lo?] where LoNm='?' order by Seq,FldNm", XXX, XXX, A)
...
Dim O$(), Cd$, GitPush, BldA1$, BldA2$, DelA1
Cd = FmtQQ("Cd ""?""", SrcPth)
BldA1 = "echo johnsoncheung>a"
BldA2 = "echo j@hns0nC4>>a"
...
Stop
End Sub
Function FfnAlreadyLoadedMsgLy(A, FilKind$, LoadTim$) As String()
Dim Sz&, Tim$, Ld$, Msg$
Sz = FfnSz(A)
Tim = FfnDTim(A)
